# Intune Win32 Packaging Framework (`IntuneApp`)
...
> **This is the previous framework.** It is being replaced by
> [Intune Package Builder](../IntunePackageBuilder/README.md), and both are in the
> repository while the replacement is finished. Nothing here has been changed by
> that work: it still builds, its 467 assertions still run, and it remains the
> one to use for anything going to production today. The two use incompatible
> configuration schemas — see the [root README](../README.md) for which to pick.
>
> Paths below are relative to this `IntuneApp\` directory, so run the commands
> from here.
...
5. Package with `IntuneWinAppUtil.exe -c . -s Install.ps1 -o Output`
